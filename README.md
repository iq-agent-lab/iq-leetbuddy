...
대소문자, 공백, `-`, `_` 모두 **자동 정규화** + **문제 번호 검색** + **leetcode.cn 지원**:
...
입력하는 동안 input 아래에 `→ two-sum 으로 정규화` 또는 `→ 문제 #1 으로 검색` **paste preview**가 실시간 표시.
...
헤더 📊 버튼 → 모달:
- **요약 카드** — 총 풀이수 / 이번 달 / 최근 7일 / 연속 풀이 일수
- **난이도 분포** — Easy / Medium / Hard 막대 (색 구분)
- **언어 분포** — 가장 많이 쓴 언어 순 막대
- **월별 풀이수** — 최근 6개월 그래프
- **최근 풀이 10개** — 번호 · 제목 · 언어 · 일자

`localStorage`에 누적 (오프라인 안전, 다른 디바이스와 sync 안 됨). 업로드 성공 시 자동 record.

### LeetCode 최근 통과 코드 자동 가져오기

step-3에 **"↩ LeetCode에서 최근 통과 코드 가져오기"** 버튼. 임베드 LeetCode 윈도우의 영속 세션 cookies(`LEETCODE_SESSION` + `csrftoken`)로 인증된 GraphQL 호출:

1. 이 문제의 최근 submission 20개 fetch
2. 첫 Accepted 골라서 submission detail에서 code 추출
3. step-3 code-input에 자동 채움 + 사용 언어로 starter lang dropdown 자동 변경

사용 흐름: 임베드 LeetCode에서 풀이 → Accepted → 메인 윈도우 복귀 → 버튼 한 번 → 코드 자동.
**복사-붙여넣기 단계 제거.**

> `submissionList`/`submissionDetails`는 LeetCode 비공식 API라 schema 변경 risk. 미인증(`임베드 LeetCode 로그인 안 함`)이거나 Accepted가 없으면 친절 에러 메시지.

### 풀이 재upload 시 알림

같은 문제를 같은 언어로 다시 풀어 업로드하려고 하면 step-3에 노란 dashed 알림:

> ⚠ **java**로 오늘 풀이를 이미 올렸어요. 업로드하면 회고가 새로 생성되고 같은 폴더의 코드/회고가 갱신됩니다 — 의도된 동작이면 그대로 진행해주세요.

`localStorage` 통계 기반 (같은 디바이스만 인지). 동작 자체는 막지 않음 — 다시 풀어서 개선한 코드로 회고 재생성하는 use case가 정상.

### 새 버전 자동 알림

부팅 시 GitHub Releases API polling. 새 버전이 있으면 footer 우측에 코랄 pill `↗ v0.x.x 사용 가능`. 클릭 → Releases 페이지 → 새 zip 다운로드.
...
- [x] **풀이 통계 dashboard** — 📊 모달 (요약 카드 4개 + 난이도/언어 분포 + 월별 그래프 + 최근 풀이). localStorage 누적
- [x] **새 버전 자동 알림** — GitHub Releases polling → footer pill (단순 polling, electron-updater의 cert 요구 회피)
- [x] **LeetCode 최근 통과 코드 자동 가져오기** — 임베드 세션 cookies로 인증된 GraphQL → step-3 자동 채움. 복사-붙여넣기 마찰 제거
- [x] **풀이 재upload 시 step-3 경고** — 같은 slug+language 풀이 있으면 dashed 알림 (localStorage 기반)
- [x] **헤더 버튼 확대 + LeetCode 로고** — 38×38 + 코랄 hover, ↗ generic icon → 실제 LeetCode favicon
